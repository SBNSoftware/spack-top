...
print_sbndaq_usage() {
    print_anydaq_usage "$(basename "$0")" "sbndaq-suite"
}
...
main() {
    if ! create_sbndaq_default_config "${BUILD_SBNDAQ_ENVFILE}"; then
        return 1
    fi
    
    run_daq_build "${BUILD_SBNDAQ_ENVFILE}" "$@"
    return $?
}
